...
export function createApp(deps: AppDeps): Hono {
	...
	app.get("/api/file/*", async (c) => {
		...
		if (rest.length === 0) {
			throw new (await import("../domain/errors.ts")).PathTraversalError(
				"empty file path",
			);
		}
	...
	app.onError((err, c) => {
		if (isAppError(err)) {
			...
			return c.json(
				errorEnvelope(err),
				statusFor(err) as 400 | 404 | 500 | 503,
			);
		}
...
export async function main(argv: string[]): Promise<number> {
	...
	await new Command()
		...
		.option("--port <port:number>", "Port to listen on (env PORT)", {
			default: undefined,
		})
		...
		.action(async (options) => {
			...
			if (isAppError(result)) {
				...
				if (err.context?.issues) {
					for (const issue of err.context.issues as Array<{
						path: (string | number)[];
						message: string;
					}>) {
						console.error(
			...
			const index = new ContentIndexService(store, {
				dotWhitelist: config.dotWhitelist,
			});
							...
							const cmd =
								Deno.build.os === "darwin"
									...
									: Deno.build.os === "windows"
										...
										: "xdg-open";
							const args =
								Deno.build.os === "windows" ? ["/c", "start", url] : [url];
